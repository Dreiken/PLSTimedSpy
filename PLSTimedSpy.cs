...
namespace PLSTimedSpy
{
    public class PLSTimedSpy : RocketPlugin<PLSTimedSpyConfiguration>
    {
        ...
        int CheckExist(UnturnedPlayer player, string DirectoryName, int Times)
        {
            ...
            if (!this.Configuration.Instance.OnlySteamId)
            {
            ...
            else
            {
                ...
                {
                    creacion[a] =
                    File.GetLastWriteTime(Path.GetFullPath(serverFolder + "/PleaseGamingFunctionalities/" + DirectoryName + "/" + player.CSteamID.ToString() + "_" + a + ".jpg"));
                }
                if (creacion[0].CompareTo(creacion[Times - 1]) < 0)
                {
                    if (this.Configuration.Instance.BackupSpy == true)
                    {
                        File.Copy(Path.GetFullPath(serverFolder + "/PleaseGamingFunctionalities/" + DirectoryName + "/" + player.CSteamID.ToString() + "_" + 0 + ".jpg"),
                       Path.GetFullPath(serverFolder + "/PleaseGamingFunctionalities/" + "BackupSpy/" + creacion[0].Month + "." + creacion[0].Day + " " + player.CSteamID.ToString() + "_" + creacion[0].Hour + "h" + creacion[0].Minute + ".jpg"));
                        File.Delete(Path.GetFullPath(serverFolder + "/PleaseGamingFunctionalities/" + DirectoryName + "/" + player.CSteamID.ToString() + "_" + 0 + ".jpg"));
                    }

                    else File.Delete(Path.GetFullPath(serverFolder + "/PleaseGamingFunctionalities/" + DirectoryName + "/" + player.CSteamID.ToString() + "_" + 0 + ".jpg"));
                    return 0;
                }
                for (int a = 0; a <= (Times - 1); a++)
                {
                    if (creacion[(a + 1)].CompareTo(creacion[a]) < 0)
                    {
                        if (this.Configuration.Instance.BackupSpy == true)
                        {
                            File.Copy(Path.GetFullPath(serverFolder + "/PleaseGamingFunctionalities/" + DirectoryName + "/" + player.CSteamID.ToString() + "_" + (a + 1) + ".jpg"),
                       Path.GetFullPath(serverFolder + "/PleaseGamingFunctionalities/" + "BackupSpy/" + creacion[a + 1].Month + "." + creacion[a + 1].Day + " " + player.CSteamID.ToString() + creacion[a + 1].Hour + "h" + creacion[a + 1].Minute + ".jpg"));
                            File.Delete(Path.GetFullPath(serverFolder + "/PleaseGamingFunctionalities/" + DirectoryName + "/" + player.CSteamID.ToString() + "_" + (a + 1) + ".jpg"));
                        }

                        else File.Delete(Path.GetFullPath(serverFolder + "/PleaseGamingFunctionalities/" + DirectoryName + "/" + player.CSteamID.ToString() + "_" + (a + 1) + ".jpg"));
                        return (a + 1);
                    }
                }
            }
